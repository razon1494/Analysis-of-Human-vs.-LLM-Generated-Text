...
% ── Tighten float spacing so figures don't leave large gaps ──────────────────
...
\title{\textbf{Hardness-Aware Robustness of LLM-Text Detection \\ Under Iterative Paraphrasing: A Mechanistic Analysis}}

\author{Mohammad Arifur Rahman \\ Anderson University \\ \texttt{rahman.arif.cse@gmail.com}}
...
attacks, with a focus on how degradation is distributed across examples of
different hardness rather than reported only as aggregate accuracy. On a
1{,}000-paragraph human-vs-Llama-3.1-8B benchmark we make four contributions.
\textbf{(1) Hardness-specific collapse.} Under a ``simplified'' paraphrase prompt,
a character-$n$-gram TF-IDF detector loses
$\Delta F_1 = -0.587$ in the Hard tertile (readability) after two rounds of
paraphrasing (0.923\,$\to$\,0.336, 95\% CI [0.308, 0.370]),
while RoBERTa-base loses only $\Delta F_1 = -0.080$.
\textbf{(2) Paraphraser diversity.} The same Hard-bucket collapse reproduces
under three paraphrasers from distinct model families
(Llama-3.1-8B, Mistral-7B, NLLB-200 back-translation), ruling out a
single-model confound.
\textbf{(3) Asymmetric flips.} The collapse is entirely a \emph{recall} failure:
\emph{all} 16 char-TF-IDF flip cases (P0 correct $\to$ P2 wrong) are
LLM texts misclassified as human (73\% of the 22 LLM examples in the Hard bucket);
zero of the 12 human Hard examples flip.
\textbf{(4) Mechanism.} Using exact linear SHAP attribution, only 13.1\% of the
detector's top-20 features survive into the paraphrased text; LLM-positive
character $n$-grams characteristic of formal vocabulary
(\texttt{`ludin'}, \texttt{`cludi'}) are replaced by short, generic
$n$-grams (\texttt{`comp'}, \texttt{`can'}, \texttt{`it'}).
Mean LLM word length collapses from 5.36 to 4.71 characters, while human word
length drops in parallel, homogenising the character signature that the
detector relied upon.
An embedding analysis (SBERT~+~UMAP) confirms the asymmetry:
the Human/LLM centroid cosine-distance in the Hard bucket erodes by only
21\% across paraphrase rounds, while char-TF-IDF $F_1$ erodes by 64\% — the
signal is still present in deep representations, only the surface encoding
collapses.
...
Detectors for LLM-generated text are now deployed in academic-integrity
systems, plagiarism workflows, and content-moderation pipelines.
Headline numbers on public benchmarks tend to be high—95\%+ accuracy is
routine for fine-tuned classifiers on in-distribution data—but practitioners
have repeatedly observed that detection accuracy degrades sharply once the
text is paraphrased, even mildly~\citep{krishna2023paraphrasing,sadasivan2023can}.
Two questions about this degradation are under-studied:
...
\begin{enumerate}
\item A \textbf{hardness-aware robustness curve} (Sec.~\ref{sec:hardness})
showing that the Hard tertile, not the Easy one, drives nearly all
degradation under \emph{simplified} paraphrasing.

\item A \textbf{detector-family asymmetry} (Sec.~\ref{sec:detector_family}):
char-TF-IDF collapses ($\Delta F_1 = -0.587$), word-TF-IDF degrades modestly
($-0.201$), RoBERTa-base barely moves ($-0.080$).

\item A \textbf{paraphraser-diversity ablation} (Sec.~\ref{sec:paraphraser_diversity}):
the Hard-bucket collapse reproduces under Mistral-7B (a different LLM
family) and under NLLB-200 (no LLM at all, just MT back-translation), ruling
out a single-model confound.

\item A \textbf{mechanistic flip analysis} (Sec.~\ref{sec:mechanism}):
on Hard-bucket flip cases, only 13.1\% of the char-TF-IDF model's
top-20 features by $|\beta|$ survive into the paraphrased text;
the surviving features are short, generic
character bigrams/trigrams; the disappearing ones are character
fragments of formal/academic vocabulary; and \emph{all} flips are
LLM-as-Human (recall failures), zero are Human-as-LLM (false positives).
\end{enumerate}

A secondary contribution (Sec.~\ref{sec:calibration}) is that
...
\emph{increases} ECE at P2-simplified by $\sim$0.12, while RoBERTa's
...
paraphrasing or scoring, to avoid circularity. We use three text-level
definitions, all computable from the text alone without any detector:
...
tertiles ($n \approx 33$ each). Bucket assignments under the three
definitions are weakly correlated (pairwise Kendall $\tau$
between $-0.28$ and $0.13$; Fig.~\ref{fig:concordance}), meaning different
examples end up in each definition's Hard bucket. Nevertheless, the
\emph{Hard-bucket $F_1$ collapse pattern} reproduces under each definition
(Fig.~\ref{fig:fig09_multiseed}): whatever subset is flagged as Hard, the
...
\citep[adapted from][]{taori2020measuring} as the mean $F_1$ across the
three rounds $\{$P0, P1, P2$\}$. A perfectly robust detector has AURC equal
to its P0 $F_1$. We summarise standard- and simplified-track AURCs in
Table~\ref{tab:aurc}.
...
\begin{subfigure}[t]{0.38\linewidth}
  ...
  \caption{Bucket-assignment concordance (Kendall $\tau$). Assignments are
  weakly correlated ($\tau$ between $-0.28$ and $0.13$), yet the collapse
  reproduces under all three—not an artifact of one definition.}
...
Table~\ref{tab:paraphraser_comparison} shows the Hard-tertile $F_1$ at P2
under each paraphraser. The qualitative pattern—char-TF-IDF collapses,
word-TF-IDF degrades modestly—survives the change of paraphraser, although
the absolute collapse magnitude is largest under Llama-as-paraphraser.
...
\centering
\small
\begin{tabular}{r p{2.0cm} p{2.0cm}}
\toprule
\textbf{rank} & \textbf{P0 (correct)} & \textbf{P2-sim (wrong)} \\
\midrule
1 & \texttt{` var'} & \texttt{` comp'} \\
2 & \texttt{` va'}  & \texttt{`comp'} \\
3 & \texttt{`ludin'} & \texttt{`omp'} \\
4 & \texttt{`cludi'} & \texttt{` it'} \\
...
P0 features are fragments of formal/academic words (\emph{variables, including}); P2-simplified features
are generic, short, everyday $n$-grams.}
\label{tab:shap_top}
\end{table}
