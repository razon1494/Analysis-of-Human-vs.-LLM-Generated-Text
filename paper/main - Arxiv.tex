...
\title{\textbf{Aggregate Metrics Hide Hard-Sample Collapse \\
       in LLM-Text Detection Under Iterative Paraphrasing}}
...
% =============================================================================
\begin{abstract}
Detectors for LLM-generated text routinely report high accuracy on
held-out benchmarks, but those aggregate scores can hide brittle behavior
on the hardest examples. We examine this failure on a balanced
1{,}000-paragraph human-vs-Llama-3.1-8B benchmark, stratifying $F_1$ by
example hardness (Flesch-Kincaid grade) \emph{before} any paraphrasing
and tracking per-bucket trajectories across two iterative paraphrase
rounds. Under simplified paraphrasing, a character-$n$-gram TF-IDF
detector loses $\Delta F_1 = -0.587$ in the Hard tertile
(0.923\,$\to$\,0.336) while RoBERTa-base loses only $-0.080$,
demonstrating that the collapse is detector-family-specific. The pattern
reproduces across three structurally distinct paraphrasers
(Llama-3.1-8B, Mistral-7B, NLLB-200 back-translation), ruling out a
single-model confound. The failure is entirely asymmetric. All 16
char-TF-IDF flip cases are LLM texts misclassified as human (73\% of LLM
Hard examples), while zero human Hard texts are misclassified in the
opposite direction. Exact linear SHAP attribution reveals that simplified
paraphrasing erases 87\% of the detector's primary features, replacing
formal academic character $n$-grams with short, generic substitutes that
carry no discriminative signal. SBERT embedding analysis indicates that
the discriminative signal between human and LLM texts is not erased by
paraphrasing but rather obscured from surface-feature detectors: semantic
cosine-distance diminishes by only 21\%, yet char-TF-IDF $F_1$ drops by
64\%, confirming that the collapse is a failure of surface encoding rather
than semantic ambiguity. We argue that hardness-stratified evaluation,
paraphraser diversity, and flip-level attribution should be standard
practice for detection robustness studies.
\end{abstract}
...
\paragraph{Why text-only hardness?}
Defining hardness via detector uncertainty (e.g., decision-boundary margin)
is tautological — examples near a decision boundary crossing it under
perturbation is mathematically expected, not a property of the data. A
reviewer-proof claim requires hardness signals the evaluated detector
cannot directly exploit. We therefore use three intrinsic text properties
(readability, type-token ratio, length) and one external-model probability
(GPT-2 perplexity). Readability is our primary signal because Flesch-Kincaid
grade level tracks syntactic complexity and vocabulary depth — both targets
of the simplified-paraphrase prompt — while remaining invisible to a TF-IDF
detector at the feature level.
...
\paragraph{Is RoBERTa's robustness real or an artifact?}
A reasonable concern is whether RoBERTa's near-perfect Hard-bucket $F_1$
reflects overfitting, topic leakage, or dataset simplicity rather than
genuine robustness. Three observations argue against this. \textbf{(i)}
Train and test paragraphs are drawn from disjoint Wikipedia articles with no
topic-matching at the generator stage; class balance is enforced at split
time. \textbf{(ii)} Hard-bucket RoBERTa $F_1$ is stable across three
independent fine-tuning seeds ($1.000 \to 0.920$ at P2-simplified, CI
$[0.865, 0.995]$), inconsistent with seed-sensitive overfitting.
\textbf{(iii)} An independent SBERT embedding analysis
(\S\ref{sec:embedding_projection}) shows Human/LLM cosine-separability
erodes only 21\% across the same paraphrase rounds where char-TF-IDF $F_1$
erodes 64\%. The most plausible reading is therefore representational, not
spurious: RoBERTa consumes a contextual feature space the simplification
attack cannot directly reach, while char-TF-IDF consumes exactly the surface
encoding the attack erases.
...
Paraphrasing damages a second downstream quality beyond classifier $F_1$:
\emph{confidence calibration}. A detector that is no longer reliably
calibrated after distribution shift cannot be safely deployed with
confidence thresholds, even when its underlying $F_1$ is above chance. We
test whether validation-fit temperature scaling — the standard calibration
patch — survives the shift induced by paraphrasing
(Table~\ref{tab:calibration}, Fig.~\ref{fig:fig18}).
